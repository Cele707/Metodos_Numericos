...
int main(int argc, char *argv[]) {
    ...
    int maxIter = 5; // cantidad max de iteraciones permitidas (gralmente. 10.000)
	int tol = 10; // exponente de la tolerancia
	char opcion = 0; // para elegir el tipo de error
	double a = 1.2, b = 1.5; // limites del intervalo
	...
	if(f(a) * f(b) > 0)
	{
		...
		for(iter = 0; error > pow(10,-tol) && iter < maxIter; iter++){
            c = (double) (a*f(b) - b*f(a)) / (f(b) - f(a)); // calculo del centro del intervalo
